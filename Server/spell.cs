...
function Spell::CastAtScreenCenter(%this, %client)
{
   ...
   if(%this.raytype !$= false)
   {
      ...
      if(%rayresult == false)
      {
         ...
         echo("Out of range");
         %this.Cast(%rayresult, %trans, "", %client.player.team, %client);
      }
      if(%rayresult != false)
      {
         ...
         {
            %trans = %rayresult SPC getWords(%client.player.getTransform(),2);
            %this.Cast(%rayresult, %trans, "", %client.player.team, %client);
         }
      }
      ...
      %this.Cast(null, null , null, %client.player.team, %client);
   }
}
   ...
   if(%this.type $= "summon")
   {
      if(%this.raytype !$= false)
      {
         ...
         if(%rayresult == false)
            echo("Out of range");
         if(%rayresult != false)
            %this.Cast(%rayresult, %rayresult SPC getWords(%client.player.getTransform(),2) , "", "mob", %client);
      }
      else{
